'<meta charset="UTF-8"><link href="prism.css" rel="stylesheet" /><title>效果展示与源代码 -CCPIRA平滑函数及应用库</title><style>*{padding: 0;margin: 0;}#pr {line-height: 1.1em;white-space: pre;overflow: auto;position: absolute;top: 0;left: 0;right: 20%;bottom: 0;margin: 2%;margin-right: 1%;padding: 10px;font-size: 0.8em;}div {position: absolute;background: rgba(255,255,255,0.6);top: 0;left: 80%;right: 0;bottom: 0;margin: 2%;margin-left: 1%;padding: 10px;}</style><script type="text/javascript">function yc() {pr.style.display=(pr.style.display=="none"?"block":"none");}window.onload = function() {eval(pr.innerText);document.body.innerHTML = document.body.innerHTML.slice(1,-1);change_eeg(document.body);change_ccstart(document.body)}</script><code id="pr" class="language-javascript">'
/*                                                      //
                                            888888      //
                                          88            //
                                          88            //
...
////////////////////////////////////////////////////////////////////////////////////////////////
                                        ...
                                        88      88    88
    88    88888888888888888888          88      88     88                     888888888888
     88   88        88                  88      88      88      888888888888            88
      88  88      88                   88       88                    88        88      88
88        88  88888888888888          88        88  8888888888       88         88      88
 88       88  88          88         888  8888888888                88          88      88
  88      88  88888888888888        8888        88                 88888888888  88      88
      88  88  88          88      88  88         88               8888      88  88888888888888
     88   88  88888888888888    88    88         88              88 88      88              88
    88    88  88    88    88          88          88            88  88      88              88
 88888    88        88                88           88               88      88              88
    88   88     88  88  88            88            88      88      88      88  8888888888  88
    88   88   88    88    88          88              88    88      8888888888              88
        88  88      88      88        88                88  88      88      88             88
     888        88  88                88                  8888      88                88  88
                  88                  88                    88                          88
...
change_n_round = 0;     //是否对结果四舍五入
change_n_tctrl = 0;     //是否为了使数组长度相近而拉伸图形
/*
函数change_rf()：
    功能简介：
        生成一个平滑的函数。
    参数：
        1.必需。一个实数（float），表示移动的距离。
        2.可选。一个对象（object），表示生成参数。有以下元素：
            I   可选。"type"，表示曲线种类。
                    默认为0。
                    0为随机。
                    1为多次函数。
                    2为椭圆函数。
                    3为三角函数。
            II  可选。"slope"，表示曲线最大/最小斜率。
                    默认为2.0。
            III 可选。"multi"，表示曲线减速倍数。
                    默认为1.0。
            IV  可选。"maxn"，表示曲线最大变形程度。
                    默认为2.6。
            V   可选。"minn"，表示曲线最小变形程度。
                    默认为1.8。
            VI  可选。"round"，表示是否对结果四舍五入。
                    默认为0。
            VII 可选。"tctrl"，表示是否为了使数组长度相近而拉伸图形。
                    默认为0。
    返回值：
        一个数组（array）。就是x为整数时函数取到的y值。
*/
function change_rf() {
    var s = parseFloat(arguments[0]), xz, i, m, minn, maxn, ifr, ctr, seta = change_fgp(arguments[1]), x = 0, bz = [];
    xz = seta.type;
    i = seta.slope;
    m = seta.multi;
    minn = seta.minn;
    ...
    switch (xz) {
        case 1:
            ...
            while (x < l) {
                bz[x] = (0 - k) * Math.pow(Math.abs(x - l), n) + s;
                x++;
            }
            break;
        case 2:
            ...
            while (x < l / 2) {
                bz[x] = k1 - ((s > 0 ? 1 : -1) * (Math.sqrt(r - x * x) / m));
                x++;
            }
            while (x < l) {
                bz[x] = (s > 0 ? 1 : -1) * (Math.sqrt(r - Math.pow((x - l), 2)) / m) + k2;
                x++;
            }
            break;
        case 3:
    ...
    if (typeof CrfGSys.prototype.tick == "undefined") {
        CrfGSys.prototype.tick = function (n) {
            ...
            for (i = 1; i <= n.non; i++) {
                ...
                t += (a[i][3].value + (i == n.non ? 0 : a[i][4].value)) / 100;
            }
            for (i = n.non + 1; i < n.changer.length; i++) delete n.changer[i];
            for (i = 1; i <= n.non; i++) s += (", rgb(" + [a[i][0].value, a[i][1].value, a[i][2].value] + ") " + (d += a[i][3].value) / t + "%" + (i != n.non ? ", rgb(" + [a[i + 1][0].value, a[i + 1][1].value, a[i + 1][2].value] + ") " + (d += a[i][4].value) / t + "%" : ""));
...
"</code><script>pr.innerHTML = pr.innerHTML.slice(2,-2);</script><div><h2>预览面板</h2><br /><button onclick='yc()'>隐藏/显示源代码</button></div><script src='prism.js'></script>"